...
plot 'csi.dat'  using ($1)*0.001:(sum [col=6:23] column(col))/18 title "{/Times O-IST - Alg. 1}"  with lp pt 6 pi 50 ps 0.8  lw 1.5  lc rgb "#00CC66",\
'' using ($1)*0.001:(sum [col=30:47] column(col))/18 title "{/Times O-DR - Alg. 3}" with lp  pt 2 pi 50 ps 0.8 lw 1.5    lc rgb "#FF3300",\
'' using ($1)*0.001:(sum [col=54:71] column(col))/18 title "{/Times O-DISTA - Alg. 5}" with lp  lw 1.5  pt 9 pi 50 ps 0.8 lc rgb "#3333FF",\
'' using ($1)*0.001:($4)*0 title "{/Times true}" with l dt "." lw 5 lc rgb "#606060",\


set yrange [0.0002:1]
set ytics 0.0002,10,0.2
set ytics add ("0.4" 0.4)
set output 'mse.pdf'
set logscale y
set key top samplen 2.5
